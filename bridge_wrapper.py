...
class YOLOv7_DeepSORT:
    ...
    def track_video(self, video: str, output: str, skip_frames: int = 0, show_live: bool = False, count_objects: bool = False, verbose: int = 0):
        ...
        if output:  # get video ready to save locally if flag is set
            ...
            output_filepath = os.path.join(output,'video.mp4')
            width = int(vid.get(cv2.CAP_PROP_FRAME_WIDTH))
            height = int(vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
            fps = int(vid.get(cv2.CAP_PROP_FPS))
            codec = cv2.VideoWriter_fourcc(*"XVID")
            out = cv2.VideoWriter(output_filepath, codec, fps, (width, height))
        ...
        while True:  # while video is running
            ...
            if count_objects:
                cv2.putText(frame, "Objects being tracked: {}".format(
                    count), (5, 35), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.5, (0, 0, 0), 2)
            ...
            for track in self.tracker.tracks:  # update new findings AKA tracks
                ...
                if verbose == 2:
                    print("Frame No:{},Tracker ID: {}, Class: {},  BBox Coords (xmin, ymin, xmax, ymax): {}".format(str(frame_num),
                        str(track.track_id), class_name, (int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))))
                    count_output_frames.append("{}/{}/{}/{}".format(str(frame_num),str(track.track_id), class_name, (int(
            ...
            if verbose == 2: ## write to file in order to perform object count
                count_filename = os.path.join(output,'tracker_output.csv')
                with open(count_filename, 'w') as f:
                    for i in count_output_frames:
                        f.write('{}\n'.format(i))
